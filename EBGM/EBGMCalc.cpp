...
Node findClosestTemplate(BunchGraph *bg, int nodeNum, Mask* masks, Image image, int flag)
{
	...
	temp->x = bg->faces[best].Nodes[nodeNum].x*scalex;
	temp->y = bg->faces[best].Nodes[nodeNum].y*scaley;
...
FaceGraph putGraph(BunchGraph *bg, Mask* masks, Image image)
{
	...
	{
		if(i==6)
			fg.Nodes[i] = findClosestTemplate(bg, i, masks, image, 1);
		else
			fg.Nodes[i] = findClosestTemplate(bg, i, masks, image, 0);
	}
	return fg;
}
